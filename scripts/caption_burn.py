...
# Layout is designed for 720x1280 and scaled to the real video size (480p Seedance clips are 480x854).
W, H = 720, 1280; MAXW = 580; HEADER_Y = 0.12; CAPTION_Y = 0.78; SCALE = 1.0
...
def header_img(lines, sz=62):
    ...
    for line in lines:
        ...
        parts = [text_img(txt, sz, px(5))] + [emoji_img(e, sz) for e in emojis]
...
def chunk_auto(words):
    ...
    for i, w in enumerate(words):
        ...
        if len(cur) == 3 or w["word"].endswith((".", ",")) or gap > 0.3 or (nxt and text_img(nxt, px(54), px(4)).width > MAXW):
...
def video_size(video):
    r = subprocess.run(["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries", "stream=width,height",
                        "-of", "csv=p=0:s=x", video], capture_output=True, text=True, check=True)
    w, h = (int(v) for v in r.stdout.strip().split("x")[:2])
    return w, h

def main():
    global W, H, MAXW, SCALE
    ...
    a = ap.parse_args()
    W, H = video_size(a.video); SCALE = W / 720; MAXW = int(580 * SCALE)
    ...
    for k, (ph, c) in enumerate(chunks):
        sz = px(54); im = text_img(ph, sz, px(4))
        while im.width > MAXW and sz > px(40): sz -= 2; im = text_img(ph, sz, px(4))
    ...
    # Check if the source video has an audio stream before mapping it
    probe = subprocess.run(
        ["ffprobe", "-v", "error", "-select_streams", "a:0", "-show_entries",
         "stream=codec_type", "-of", "csv=p=0", a.video],
        capture_output=True, text=True)
    has_audio = bool(probe.stdout.strip())
    audio_args = ["-map", "0:a", "-c:a", "copy"] if has_audio else []
    subprocess.run(["ffmpeg", "-v", "error", "-y", "-i", a.video, *inputs, "-filter_complex_script", f"{tmp}/filter.txt",
                    "-map", f"[v{len(chunks)}]", *audio_args, "-c:v", "libx264", "-crf", "18", "-preset", "medium",
