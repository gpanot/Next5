...
export const renderEmail = (content: EmailContent): { subject: string; html: string; plain: string } => {
  ...
  const cta = content.cta
    ? `<a href="${escape(content.cta.url)}" style="display:inline-block;margin:8px 0 20px;background:${THEME.accent};color:${THEME.accentInk};text-decoration:none;padding:12px 22px;border-radius:12px;font-size:15px;font-weight:600">${escape(content.cta.label)}</a>`
    : '';
