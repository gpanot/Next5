...
export const UgcVideoMock = ({ video, className = '', badge = true }: { video: UgcVideoExample; className?: string; badge?: boolean }) => (
  <div className={`relative mx-auto w-full max-w-[220px] sm:max-w-[260px] ${className}`}>
    <PhoneFrame>
      <div className="relative aspect-[9/16] w-full bg-app-sunken">
        ...
        <span className="absolute left-1/2 top-1/2 flex h-11 w-11 -translate-x-1/2 -translate-y-1/2 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-white/85 shadow-md" aria-hidden>
        ...
        <div className="absolute inset-x-3 top-[64%] flex flex-col items-center gap-1" aria-label={`On-screen captions: ${video.captions.join(' ')}`}>
          {video.captions.map((line) => (
            <span key={line} className="rounded-md bg-white px-2 py-0.5 text-center text-[11px] font-extrabold leading-tight sm:text-[13px] text-black shadow-sm">{line}</span>
          ))}
        </div>
        <div className="absolute bottom-3 right-2 hidden flex-col items-center gap-3 text-white sm:flex" aria-hidden>
          <Heart className="h-5 w-5" /><MessageCircle className="h-5 w-5" /><Share2 className="h-5 w-5" />
        </div>
        <div className="absolute bottom-3 left-3 hidden flex-col gap-0.5 text-white sm:flex">
