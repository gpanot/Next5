...
export const ScoreCard = ({ score, details }: { score: number; details: ScoreDetailsDto | null }) => (
  <section aria-label="Scroll-Stop Score" className="flex flex-col gap-3">
    <div className="flex items-baseline justify-between gap-3">
      <p className="text-[13px] font-medium text-app-muted">Scroll-Stop Score</p>
      <p className="text-[13px] text-app-muted"><span className="text-[26px] font-semibold tabular-nums text-app-ink">{score}</span>/100 · {BAND_LABEL[scoreBand(score)]}</p>
    </div>
    {details && (
      <>
