...
import { Check, ChevronDown, Link2, PartyPopper, X } from 'lucide-react';
...
export const QuickstartCard = ({ me }: { me: MeDto }) => {
  ...
  // Also treat as done when autopilot is already enabled server-side.
  const step3Done = saved.step3Done || Boolean(calendarData?.schedule.autopilot);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!isImportBusy(res.listing)) {
        // Already ready (e.g. same ZPID re-imported).
        save({ step2Done: true });
        setImportBusy(false);
      } else {
        setActiveListing(res.listing);
        // importBusy stays true; polling will resolve it.
      }
    } catch (err) {
  ...
  const handleAutofill = async () => {
    ...
    try {
      ...
      await apiFetch<CalendarDto>('/api/app/calendar', {
        method: 'PUT',
        json: {
      ...
      setAutofillError(err instanceof ApiError ? err.message : 'Could not enable autopilot. Try again.');
    } finally {
            ...
            {!step2Done && (
              <div className="mt-3 flex flex-col gap-2">
                ...
                {importBusy && !importError && (
                  <p className="text-[12px] text-app-muted">Getting your photos from Zillow — takes ~20 seconds…</p>
                )}
