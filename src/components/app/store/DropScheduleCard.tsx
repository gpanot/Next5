      ...
      <CardBody className="flex flex-col gap-5">
        ...
        {form.active && (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field label="Model and scene" htmlFor="drop-set"><Select id="drop-set" value={form.setId ?? ''} onChange={(e) => set('setId', e.target.value || null)}><option value="">Latest used</option>{(sets.data?.sets ?? []).map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</Select></Field>
