        ...
        <div className="flex flex-col gap-3">
          ...
          {value.brandColors.length > 0 && (
            <div className="flex gap-4">
              {[0, 1].map((i) => (
                <ColorInput key={i} label={`Brand colour ${i + 1}`} value={value.brandColors[i] ?? '#0b0b0c'} onChange={(c) => { const next = [...value.brandColors]; next[i] = c; onChange({ ...value, brandColors: next.slice(0, 2) }); }} />
              ))}
            </div>
          )}
        </div>
