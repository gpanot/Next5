...
type Props = { templates: readonly SetTemplateDto[]; selected: readonly string[]; onToggle: (id: string) => void };
...
const StyleTile = ({ template, on, onToggle }: { template: SetTemplateDto; on: boolean; onToggle: () => void }) => {
...
export const StylePager = ({ templates, selected, onToggle }: Props) => {
      ...
      >
        {pages.map((items, index) => (
          <div key={index} className="grid w-full shrink-0 snap-start grid-cols-3 gap-x-2 gap-y-3 px-1 py-1" aria-label={`Styles page ${index + 1} of ${pages.length}`}>
            {items.map((t) => <StyleTile key={t.id} template={t} on={selected.includes(t.id)} onToggle={() => onToggle(t.id)} />)}
          </div>
        ))}
      </div>
      {pages.length > 1 && (
