...
export const SetEditor = ({ existing }: SetEditorProps) => {
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      const body = { product, name, ...(product === 'brand' ? style : {}) };
      if (existing) await apiFetch(`/api/app/sets/${existing.id}`, { method: 'PATCH', json: body });
      else await apiFetch('/api/app/sets', { method: 'POST', json: { ...body, templateId, modelRef: product === 'shop' ? 'me' : undefined } });
      refresh();
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex justify-end gap-2">
        ...
        <AppButton size="lg" loading={busy} disabled={!name || (!existing && !templateId)} onClick={save}>{existing ? 'Save changes' : `Create ${noun}`}</AppButton>
      </div>
    </div>
  );
};
