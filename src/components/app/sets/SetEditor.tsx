...
/** Brand: create (no `existing`) or edit a style. Template can't change after creation. Shop uses ShopModelEditor. */
export const SetEditor = ({ existing }: SetEditorProps) => {
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      const body = { product, name: name || template?.name || '', ...style };
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap justify-end gap-2">
        ...
        <AppButton size="lg" loading={busy} disabled={!(name || template?.name) || (!existing && !templateId)} onClick={save}>{existing ? 'Save changes' : `Add ${noun}`}</AppButton>
      </div>
