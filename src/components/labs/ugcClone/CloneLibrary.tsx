...
function CloneCard({
  ...
  useEffect(() => {
    ...
    let stopped = false;
    const tick = async () => {
      const res = await client.request<{ video?: CloneVideoDto }>(`/ugc-lab/clone/library/${video.id}`,
      ).catch(() => null);
      if (!stopped && res?.ok && res.data.video) onChangeRef.current(res.data.video);
    };
    const id = window.setInterval(() => void tick(), POLL_MS);
    return () => { stopped = true; window.clearInterval(id); };
  }, [client, video.id, video.status]);
  ...
  async function handleDelete() {
    setDeleting(true);
    setDeleteError('');
    const res = await client.request(`/ugc-lab/clone/library/${video.id}`, { method: 'DELETE' }).catch(() => null);
    setDeleting(false);
...
export function CloneLibrary() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError('');
    const res = await client.request<{ videos?: CloneVideoDto[] }>('/ugc-lab/clone/library',
    ).catch(() => null);
    setLoading(false);
