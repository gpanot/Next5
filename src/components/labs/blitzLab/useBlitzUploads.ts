...
export function useBlitzUploads({ setAssets, onKeyReplaced }: Options) {
  ...
  const run = useCallback(async (localKey: string, type: BlitzUploadType) => {
    ...
    try {
      const saved = await uploadBlitzAsset(client, type, entry.file, (p) => patch(localKey, { progress: p }));
  ...
  }, [client, setAssets, onKeyReplaced, patch]);
