...
export const useUgcVideos = () => {
  const client = useLabClient();
  const { data, error, loading, refresh } = useLabQuery<{ videos: UgcVideoDto[]; etas: Partial<Record<number, UgcEta>> }>('/ugc-lab/videos');
  ...
  return {
    ...
    remove: (id: string) => {
      ...
      void client.request(`/ugc-lab/videos/${id}`, { method: 'DELETE' });
    },
  };
};
