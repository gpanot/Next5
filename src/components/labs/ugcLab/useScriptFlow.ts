 ...
 */
export const useScriptFlow = (initialHook: string, onDescribed: (character: UgcCharacterDto) => void) => {
  ...
  async function describe(character: UgcCharacterDto): Promise<UgcCharacterDto> {
    ...
    setBusy('describe');
    const res = await client.request<{ scene?: UgcScene }>('/ugc-lab/describe', { json: { characterId: character.id } })
      .catch(() => null);
  ...
  async function writeScripts(character: UgcCharacterDto, hookOverride?: string, industry?: string) {
    ...
    setBusy('scripts');
    setError('');
    const res = await client.request<{ scripts?: ScriptOption[]; suggestedVideoContext?: string }>('/ugc-lab/scripts',
