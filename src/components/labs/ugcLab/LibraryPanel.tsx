...
const LegacyImport = ({ onImported }: { onImported: (videos: UgcVideoDto[]) => void }) => {
  ...
  async function runImport() {
    setImporting(true);
    setError('');
    const res = await client.request<{ videos?: UgcVideoDto[] }>('/ugc-lab/videos/import', { json: { items: runs } })
      .catch(() => null);
    setImporting(false);
...
export function LibraryPanel() {
  const { videos, etas, error, loading, reload, update, remove, addMany } = useUgcVideos();
  ...
  return (
    ...
    >
      <LegacyImport onImported={addMany} />
      ...
      {videos.length > 0 && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {videos.map((v) => <VideoCard key={v.id} video={v} eta={etas[v.durationSec]} onChange={update} onDelete={remove} />)}
        </div>
      )}
    </Section>
  );
