...
export function VideoPanel({ selection, voiceKey, onOpenLibrary }: VideoPanelProps) {
  ...
  async function updatePrompt() {
    ...
    const res = await client.request<{ context?: string }>('/ugc-lab/suggest-prompt', {
      json: {
  ...
  async function submit(confirmOverBudget: boolean) {
    ...
    setSubmitError('');
    setConfirmCost(null);
    const res = await client.request<GenerateResponse>('/ugc-lab/generate', {
      json: {
        ...
        {videos.length > 0 && (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {videos.slice(0, RECENT_COUNT).map((v) => (
              <VideoCard key={v.id} video={v} eta={etas[v.durationSec]} onChange={update} onDelete={remove} />
            ))}
          </div>
        )}
      </Section>
