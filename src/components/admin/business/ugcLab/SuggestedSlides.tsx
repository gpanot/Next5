 ...
 *
 * With a niche and a token it asks the LLM to rewrite the template's generic
...
type SuggestedSlidesProps = {
  ...
  niche?: string;
  /** Admin token. Absent disables generation. */
  token?: string;
  videoId: string;
  hook?: string;
  transcript?: string;
...
export function SuggestedSlides({
  ...
  const canGenerate = Boolean(trimmedNiche && token);
  ...
  useEffect(() => {
    if (!requestKey || !token) return;
    let active = true;
    getNicheSlides(token, { niche: trimmedNiche, templateId, videoId, hook, transcript })
      .then((result) => {
    ...
    return () => { active = false; };
  }, [requestKey, token, trimmedNiche, templateId, videoId, hook, transcript]);
  ...
  const regenerate = () => {
    ...
    forgetNicheSlides({ niche: trimmedNiche, templateId, videoId, hook, transcript });
    setNonce((n) => n + 1);
  };
