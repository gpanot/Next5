...
const useVideoPolling = (token: string, video: UgcVideoDto, onChange: (video: UgcVideoDto) => void) => {
  ...
  useEffect(() => {
    ...
    let stopped = false;
    const tick = async () => {
      const res = await ugcRequest<{ video?: UgcVideoDto }>(token, `/api/admin/ugc-lab/videos/${video.id}`).catch(() => null);
...
export function VideoCard({ token, video, eta, onChange, onDelete }: VideoCardProps) {
  ...
  useVideoPolling(token, video, onChange);
  ...
  async function burnCaptions() {
    setBurning(true);
    setActionError('');
    const res = await ugcRequest<{ video?: UgcVideoDto }>(token, `/api/admin/ugc-lab/videos/${video.id}/captions`, { method: 'POST' })
      .catch(() => null);
    setBurning(false);
