 ...
 * CORS rule on the bucket), send the file through /api/admin/blitz/upload.
...
const uploadDirect = async (token: string, type: BlitzUploadType, file: File, onProgress: Progress) => {
  const res = await blitzApi.getUploadUrl(token, type, file.name);
...
const uploadViaServer = async (token: string, type: BlitzUploadType, file: File, onProgress: Progress) => {
  const form = new FormData();
  form.append('file', file);
  form.append('type', type);
  const res = await sendWithProgress('POST', '/api/admin/blitz/upload', form, { Authorization: `Bearer ${token}` }, onProgress);
...
export const uploadBlitzAsset = async (
  ...
  let r2Key: string;
  try {
    r2Key = await uploadDirect(token, type, file, onProgress);
  } catch (err) {
    if (!(err instanceof NetworkError)) throw err;
    console.warn('[blitz] Direct R2 upload failed, using server fallback:', err.message);
    onProgress(0);
    r2Key = await uploadViaServer(token, type, file, onProgress);
  }
  const res = await blitzApi.registerAsset(token, { type, r2Key, name: file.name });
