...
export const PromiseTab = ({ token }: { token: string }) => {
  ...
  if (!data?.claims.length) return <p className="text-[13px] text-muted">No promise claims yet.</p>;
  const wins = data.claims.filter((c) => c.outcome === 'won');
  return (
    <div className="flex flex-col gap-4">
      <p className="text-[13px] text-muted">{wins.length} of {data.claims.length} claims beat their feed · average lift {Math.round(wins.reduce((s, c) => s + (c.afterAverage - c.beforeAverage) / Math.max(1, c.beforeAverage), 0) / Math.max(1, wins.length) * 100)}% on wins</p>
      {message && <p className="text-[13px] text-ink">{message}</p>}
      <ul className="flex flex-col gap-3">
        {data.claims.map((c) => (
          <li key={c.id} className="rounded-xl border border-line bg-white p-4 text-[13px]">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              <p className="font-medium text-ink">{c.workspace} · {c.email} · {c.product}</p>
              <p className="text-muted">{new Date(c.createdAt).toLocaleDateString()} · {c.status}</p>
            </div>
            <p className="mt-1 text-ink">
              ...
              <span className={c.outcome === 'won' ? 'text-emerald-700' : 'text-accent-strong'}>{c.outcome === 'won' ? 'beat their feed' : 'missed — free month owed'}</span>
              {c.sharePermission && <span className="ml-2 rounded-full bg-emerald-50 px-2 py-0.5 text-emerald-700">OK to share</span>}
            </p>
            {c.links.length > 0 && <ul className="mt-1 flex flex-wrap gap-2">{c.links.map((l) => <li key={l}><a href={l} target="_blank" rel="noreferrer" className="text-accent-strong underline">{new URL(l).hostname}</a></li>)}</ul>}
            {c.note && <p className="mt-1 text-muted">{c.note}</p>}
            {c.status === 'pending' && (
              <div className="mt-2 flex gap-2">
                <button type="button" onClick={() => void decide(c, 'grant')} className="rounded-lg bg-ink px-3 py-1 text-white">Grant free month</button>
                <button type="button" onClick={() => void decide(c, 'decline')} className="rounded-lg px-3 py-1 text-accent-strong hover:bg-surface-alt">Decline</button>
              </div>
            )}
          </li>
        ))}
