  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex gap-2">
        {STATES.map((s) => <button key={s} onClick={() => setState(s)} className={`rounded-full px-3 py-1 text-[12px] capitalize ${state === s ? 'bg-ink text-white' : 'bg-white text-muted ring-1 ring-line'}`}>{s || 'All'}</button>)}
      </div>
          ...
          <tbody>
            {(data?.payments ?? []).map((p) => (
              <tr key={p.id} className="border-t border-line">
                <td className="px-4 py-3 text-muted">{p.createdAt.slice(0, 16).replace('T', ' ')}</td>
                ...
                <td className="px-4 py-3">{(p.state === 'pending' || p.state === 'expired' || p.state === 'underpaid') && <button onClick={() => markPaid(p)} className="rounded-lg px-2 py-1 text-[12px] text-accent-strong hover:bg-surface-alt">{p.provider === 'request' ? 'Activate' : 'Mark paid'}</button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
