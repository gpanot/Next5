...
### 3.1 Images first
- [ ] Generate every image in `04-image-prompts.md` sections **A, B, C1–C14** and record them in `public/images/manifest.json`.
- [ ] For C2/C4/C6 (after images): use fallback prompts now; add a TODO in `04-image-prompts.md` to replace with pipeline output after P8.
- [ ] Review each image for artefacts (hands, text, logos). Regenerate failures.

### 3.2 Shared marketing layout (`src/components/marketing/shared/`)
- [ ] `MarketingHeader.tsx` — per `03-ux-ui.md` §1 (logo, For professionals `/brand`, For shops `/shop`, Pricing `/pricing`, Log in `/app`, CTA contextual: on `/brand` → "Start free", on `/shop` → "Try it free", else "Get started" → `/#products`). Sticky, blur after scroll (reuse `useScrolled`), mobile menu sheet.
- [ ] `MarketingFooter.tsx` — Products, Pricing, Next5 Photos (`/photos`), Log in, Legal (links added in P11), © line.
- [ ] `StickyMobileCta.tsx` (business version) — appears after hero scrolls out.
- [ ] `SectionHeader.tsx` (eyebrow `label-caps`, Cormorant H2 sentence case, sub), `FaqAccordion.tsx` (keyboard accessible, `aria-expanded`), `FinalCtaBand.tsx`, `GuaranteeRow.tsx`, `PrivacyRow.tsx`, `SampleBadge.tsx` ("Sample made with Next5").
- [ ] `PhoneFrame.tsx`, `BrowserFrame.tsx` — pure CSS device frames for UI mocks (no real platform UI).
- [ ] `app/(marketing)/layout.tsx` route group wrapping `BusinessSurface` + header/footer for `/brand`, `/shop`, `/pricing` (and `/` at launch). Check Next 16 docs for route groups + metadata.

### 3.3 Content modules (`src/content/business/`)
Keep copy out of components (English only):
- [ ] `home.ts`, `brand.ts`, `shop.ts`, `pricing.ts`, `faq.ts` — typed objects with the copy from `01-product-spec.md` §6 and the section lists in `03-ux-ui.md` §5. Image paths reference manifest keys.

### 3.4 Home (`src/components/marketing/home/`, `app/(marketing)/home-preview/page.tsx` until launch)
- [ ] `HomeHero`, `ProductChooser` (two cards), `SharedHowItWorks`, `GuaranteeRow`, `PrivacyRow`, `FaqAccordion`, `FinalCtaBand`, tertiary link to `/photos`.
- [ ] Build it at `/home-preview` behind the flag; the swap of `app/page.tsx` happens in P11 launch.

### 3.5 Brand page (`src/components/marketing/brand/`, `app/(marketing)/brand/page.tsx`)
Sections in the order of `03-ux-ui.md` §5.2:
- [ ] `BrandHero` (with `PhoneFrame` 3×3 grid of B3–B16 images), `OldWayComparison`, `BrandHowItWorks`,
...
- [ ] `ShopHero` with `BeforeAfterSlider` (pointer + keyboard: arrow keys move 5%; `role="slider"`, `aria-valuenow`; chips switch Dress/Set/Bag),
...
- [ ] `ProductToggle` (URL `?for=professionals|shops`, default professionals), `TermToggle` (`SegmentedControl` 1/3/6, default 3),
...
- [ ] Unit test for any price-rendering helper (`$51` / `$132` / `Save 20%`).
