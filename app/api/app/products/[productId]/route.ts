...
const owned = async (userId: string, productId: string) => {
  const product = await prisma.product.findFirst({ where: { id: productId, workspace: { ownerUserId: userId } } });
...
/** DELETE — archives the product (existing photos stay in the library; a store sync won't bring it back). */
export const DELETE = authedRoute<Ctx>(async (_req, session, ctx) => {
  const { productId } = await ctx.params;
  const product = await owned(session.userId, productId);
  await prisma.product.update({ where: { id: product.id }, data: { archivedAt: new Date(), archivedBySeller: true } });
